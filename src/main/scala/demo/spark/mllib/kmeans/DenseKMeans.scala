package demo.spark.mllib.kmeans

import demo.spark.mllib.AbstractParams
...
object DenseKMeans {
  ...
  def run(params: Params) {
    val conf = new SparkConf().setAppName(s"DenseKMeans with $params").setMaster("local[4]")
    ...
    val examplesWithPredictions = examples.map {x=> (x,model.predict(x)) }    
    examplesWithPredictions.collect.foreach(println)
    sc.stop()
  }
}
