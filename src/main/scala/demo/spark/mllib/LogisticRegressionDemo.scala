...
object LogisticRegressionDemo {
  ...
  def main(args: Array[String]) {
    val data: RDD[LabeledPoint] = MLUtils.loadLibSVMFile(sc, "c://sample_libsvm_data.txt") //读取数据文件
    ...
    val predictionAndLabel = data.map(line=>{
      val prediction = model.predict(line.features)
      (prediction, line.label)
    })
    
    val MSE = predictionAndLabel.map{case(p, v)=>{
      math.pow((p-v),2)
    }}.mean()
    println(MSE)
