...
public class RedisUtil implements Serializable {
    ...
    public RedisUtil() {
        String JEDIS_IP = "172.21.14.25";
        int JEDIS_PORT = 6379;
        String JEDIS_PASSWORD = null;
        if (pool == null) {
            JedisPoolConfig config = new JedisPoolConfig();
            // 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
            config.setTestOnBorrow(true);
            config.setMaxTotal(1000);
            pool = new JedisPool(config, JEDIS_IP, JEDIS_PORT, 100000);
        }
    }
    ...
    public RedisUtil(String ipAndPort) {
        String[] ipPort = StringUtils.splitByWholeSeparatorPreserveAllTokens(ipAndPort, ",");
        if (pool == null) {
            JedisPoolConfig config = new JedisPoolConfig();
            config.setTestOnBorrow(false);
            config.setMaxTotal(1000);
            pool = new JedisPool(config, ipPort[0], Integer.valueOf(ipPort[1]), 100000);
        }
    }
     ...
     * @param port 端口
     */
    public RedisUtil(String ip, int port, String passport) {
        if (pool == null) {
            JedisPoolConfig config = new JedisPoolConfig();
            config.setTestOnBorrow(false);
            config.setMaxTotal(200);
            pool = new JedisPool(config, ip, port, 100000, passport);
        }
    }
     ...
     * @return 返回list的value个数
     */
    public Long lpush(byte[] key, byte[] strs) {
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
     ...
     * @return 返回list的value个数
     */
    public Long rpush(byte[] key, byte[] strs) {
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
    ...
    public List<byte[]> lrangeList(byte[] key) {
        Jedis jedis = null;
        List<byte[]> res = null;
        try {
            jedis = pool.getResource();
