 ...
 */
public class WordCount_window_append4 {
...
//        row.explain(true);
        ...
        row.printSchema();
        row.createOrReplaceTempView("online");

        StreamingQuery query = spark.sql("SELECT  WINDOW(timestamp, \"10 seconds\",\"4 seconds\"), COUNT(*) FROM online GROUP BY  WINDOW(timestamp, \"10 seconds\",\"4 seconds\")")
                .writeStream()
