...
public class OnelineLearningExpClickJoin2 {
    ...
    public static void main(String args[]) throws InterruptedException, StreamingQueryException {
        ...
        Dataset<Row> evDataSet = spark.sql("select * from online where event='EV'").withWatermark("time", "10 seconds");
        ...
        StreamingQuery query =
                spark.sql("select *,if(click.doc is null,0,1) from ev left outer join click on ev.deviceid=click.deviceid and ev.doc = click.doc and click.time >= ev.time and click.time <= ev.time + INTERVAL 10 seconds " )
