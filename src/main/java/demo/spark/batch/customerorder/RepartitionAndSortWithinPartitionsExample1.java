...
/**
 * 分区并且按照key进行排序：repartitionAndSortWithinPartitions
 * 这个方法比先分区，然后再排序更有效率
 *
 *
...
public class RepartitionAndSortWithinPartitionsExample1 implements Serializable {
    ...
    static List<Tuple2<Integer, Integer>> buildSampleList() {
        ...
        pairs.add(new Tuple2<>(8, 1));
        pairs.add(new Tuple2<>(2, 12));
        //
        pairs.add(new Tuple2<>(3, 9));
    ...
    public static void main(String[] args) {
        ...
        List<List<Tuple2<Integer, Integer>>> partitions = repartitioned.glom() //将每个分区的数据转为list，返回JavaRDD<List<>>
                                                                       .collect();
