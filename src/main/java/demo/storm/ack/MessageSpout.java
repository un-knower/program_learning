...
public class MessageSpout implements IRichSpout {
	...
	public void nextTuple() {
		if(index < subjects.length){
			...
			collector.emit(new Values(sub), index); //添加msgId预防容错
			index++;
		}
	}
	...
	public void fail(Object msgId) {
		...
		collector.emit(new Values(subjects[(Integer) msgId]), msgId); //如果实现IBatchSpout，没有重写的fail方法，需要在emitBatch中实现相关方法
