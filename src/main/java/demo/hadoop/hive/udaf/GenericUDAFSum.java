...
public class GenericUDAFSum extends AbstractGenericUDAFResolver{
	static final Log LOG = LogFactory.getLog(GenericUDAFSum.class.getName()); //创建log对象，用来写入警告和错误到hive的log	
	@Override
	   ...
	   //这个方法返回了UDAF的返回类型，这里确定了sum自定义函数的返回类型是Long类型
		...
		public void merge(AggregationBuffer agg, Object partial)
			...
			if (partial != null) {
				...
				//myagg.sum+=(long) inputOI.getPrimitiveJavaObject(partial);不可以，会报错
				myagg.empty = false;
			}
		}
