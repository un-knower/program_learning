...
public class ReservoirSamplerInputFormat<K extends Writable, V extends Writable>
  ...
  public static class ReservoirSamplerRecordReader<K extends Writable, V extends Writable>
    ...
    private final RecordReader<K, V> rr; //使用封装的RecordReader从数据源中读取记录
    private final int numSamples; //从数据源中提取的样本数量
    private final int maxRecords; //从数据源中读取记录的上限数量，避免读取整个数据源
    ...
    public void initialize(InputSplit split,
      ...
      for (int i = 0; i < maxRecords; i++) {
        ...
        if (keys.size() < numSamples) { //如果没有收集样本的目标数量，那么将当前记录添加到样本个中
          keys.add(WritableUtils.clone(key, conf));
          values.add(WritableUtils.clone(val, conf));
        } else {
          int r = rand.nextInt(i);
          if (r < numSamples) { //当收集到了样本的最小数量，使用蓄水池算法确定是否需要使用当前的记录更新现存的样本
